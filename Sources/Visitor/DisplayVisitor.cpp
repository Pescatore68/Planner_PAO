...
DisplayVisitor::DisplayVisitor()
    : textSummary(""), tagText(""), isRoutineType(false), hasCheckableStatus(false),
    isChecked(false), isWriteMode(false), newValueToSet(false), currentViewDate(date::today()) {}

void DisplayVisitor::processTag(const tag* t) {
...
int DisplayVisitor::getOffset(const date& start) const {
...
void DisplayVisitor::setViewDate(const date& d) { currentViewDate = d; }

void DisplayVisitor::setWriteMode(bool value) {
    isWriteMode = true;
    newValueToSet = value;
}

std::string DisplayVisitor::getSummary() const { return textSummary; }
bool DisplayVisitor::isRoutine() const { return isRoutineType; }
bool DisplayVisitor::isCheckable() const { return hasCheckableStatus; }
bool DisplayVisitor::getCheckedState() const { return isChecked; }
QColor DisplayVisitor::getTagColor() const { return tagColor; }

void DisplayVisitor::applyToLayout(QVBoxLayout* targetLayout, QWidget* parent) const {
...
void DisplayVisitor::visit(Event& e) {
    isRoutineType = false;
...
void DisplayVisitor::visit(Reminder& r) {
    isRoutineType = false;
    hasCheckableStatus = false;
...
void DisplayVisitor::visit(task& t) {
    isRoutineType = false;
...
void DisplayVisitor::visit(project& p) {
    isRoutineType = false;
    hasCheckableStatus = true;
...
void DisplayVisitor::visit(Routine& r) {
    isRoutineType = true;
    hasCheckableStatus = true;
